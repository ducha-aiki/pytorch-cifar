...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/resnet18_quick_gor_init_var.t7')
        best_acc = acc
...
def gor_filter_loss(model):
    ...
    for param in model.parameters():
        ...
        if len(sh) > 1:
            ...
            dist_matrix = cos_dist(weights,weights)**2
            eye = torch.autograd.Variable(1.0 - torch.eye(dist_matrix.size(1))).cuda()
            dist_without_min_on_diag = eye * dist_matrix
            max_neg_a = torch.sqrt(torch.max(dist_without_min_on_diag,1)[0] + 1e-8)
            if loss is None:
                loss = max_neg_a.mean()
            else:
                loss+=  max_neg_a.mean()
            loss += (torch.sqrt(torch.sum(weights * weights, 1)).mean() - 1.0)**2;
    return loss
net.train()
