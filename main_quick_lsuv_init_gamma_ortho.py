...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/resnet18_quick_gamma_ortho_lsuv_init.t7')
        best_acc = acc
...
net = LSUVinit(net,inputs, needed_std = 1.0, std_tol = 0.1, max_attempts = 10, do_orthonorm = True, gamma = True, cuda = use_cuda)
net.train()
