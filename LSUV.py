...
def svd_gamma_orthonormal(w):
    shape = w.shape
    if len(shape) < 2:
        raise RuntimeError("Only shapes of length 2 or more are supported.")
    flat_shape = (shape[0], np.prod(shape[1:]))
    a = np.random.gamma(0.1, 1.0, flat_shape) * np.sign(np.random.normal(0,1,flat_shape)).astype(np.float32)#w;
...
def remove_hooks(hooks):
    for h in hooks:
        h.remove()
    return
def orthogonal_gamma_weights_init(m):
    if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
        if hasattr(m, 'weight_v'):
            w_ortho = svd_gammma_orthonormal(m.weight_v.data.cpu().numpy())
            ...
            w_ortho = svd_gamma_orthonormal(m.weight.data.cpu().numpy())
            #print w_ortho 
            ...
            try:
                ...
                pass
    return
def gamma_weights_init(m):
    if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
        if hasattr(m, 'weight_v'):
            w_ortho = gamma1(m.weight_v.data.cpu().numpy())
            m.weight_v.data = torch.from_numpy(w_ortho)
            try:
                nn.init.constant(m.bias, 0)
            except:
            ...
            #nn.init.orthogonal(m.weight)
            w_ortho = gamma1(m.weight.data.cpu().numpy())
            #print w_ortho 
            #m.weight.data.copy_(torch.from_numpy(w_ortho))
            m.weight.data = torch.from_numpy(w_ortho)
            try:
    ...
    if (isinstance(m, nn.Conv2d)) or (isinstance(m, nn.Linear)):
        if gg['counter_to_apply_correction'] < gg['hook_position']:
            ...
            if hasattr(m, 'weight_g'):
                ...
                #nn.init.constant(m.bias, float(-gg['act_dict'].mean() *float(gg['current_coef'])/2.))
...
def LSUVinit(model,data, needed_std = 1.0, std_tol = 0.1, max_attempts = 10, do_orthonorm = True, gamma = False, cuda = False):
    model.eval();
    if cuda:
        model = model.cuda()
        data = data.cuda()
    ...
    if gamma and not do_orthonorm:
        model.apply(gamma_weights_init)
        print 'Gamma done'
        if cuda:
            model = model.cuda()
    if gamma and do_orthonorm:
        model.apply(orthogonal_gamma_weights_init)
